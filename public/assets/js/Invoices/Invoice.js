    ...
    $("#example").DataTable({
        ...
        drawCallback: function (settings) {
            ...
            if (value_type == 'filter' && response != undefined) {
                var getSpinnerType = spinerType.split('|');
...
$(document).ready(function () {
    ...
    $('#apply_filters').click(function () {
        param.customer_acno = $("#customer_acno").val();

        if ((param.customer_acno == '')) {
            notify("warning", "Oh snap!", "Please select filter.");
        } else {
    ...
    $("#resetFilter").click(function () {
        ...
        $('#customer_acno').val('').trigger('change');;
        param.customer_acno = '';
        param.value_type = "filter";
    ...
    $("#search-btn").click(function (e) {
        var forms = $("#invoice_form");
        initLoader('search-btn', '', 'btn-secondary-orio');
        forms.parsley().validate();
        if (!forms.parsley().isValid()) {
            destroyLoader('search-btn', 'Search', 'btn-secondary-orio')
            return false;
        }
        e.preventDefault();
        if (forms.parsley().isValid()) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.status == 1) {
                        $('#display-details').html(result.payload);
                        destroyLoader('search-btn', 'Search', 'btn-secondary-orio')
    ...
    $("#save-btn").click(function (e) {
        ...
        if (forms.parsley().isValid()) {
            ...
            formdata.append('_token', $('meta[name="csrf-token"]').attr('content'));
            let detail = [];
            $('#display-details tr').each(function () {
                let $row = $(this);
                let consignment = $row.find("td.cn").text().trim();
                let oldServiceCharges = $row.find("td.old_service_charges").text().trim();
                let oldRtoCharges = $row.find("td.old_rto_charges").text().trim();
                var service_charges_inp = $(this).find("input[name='service_charges[]']")
                if(service_charges_inp.length === 0){
                    var serviceCharges = oldServiceCharges;                                        
                }else{
                    var serviceCharges = service_charges_inp.val();                    
                }
                var rto_charges_inp = $(this).find("input[name='rto_charges[]']")
                if(rto_charges_inp.length === 0){
                    var rtoCharges = oldRtoCharges;                                        
                }else{
                    var rtoCharges = rto_charges_inp.val();                    
                }
                if(rtoCharges == ''){
                    notify("danger","RTO charges for shipment "+consignment+" cant be empty");
                    destroyLoader('save-btn', title, 'btn-orio')
                    return false;
                }
                detail.push({
                    ...
                    old_service_charges: oldServiceCharges,
                    rto_charges: rtoCharges,
                    old_rto_charges: oldRtoCharges,
                });
            });
                    ...
                    success: function (result) {
                        if (result.status == 1) {
                            ...
                            swal.fire({
                                ...
                                text: "Invoice sheet no # " + result.payload.sheet_no,
                                confirmButtonClass: "btn-success",
                                type: "success",
                            });
                            window.setTimeout(function () {
                                window.location.href = BASEURL + 'cbc-invoice';
                            }, 1000);
                        } else {
    ...
    $(document).on('click', '.rem_row', function () {
        ...
        if ($('#display-details tr').length === 0) {
            $("#save-btn").addClass('d-none');
        }
        count_total()
    });
...
$(document).on('click', '.check_box', function () {
    ...
    $row.find('input[name="service_charges[]"], input[name="rto_charges[]"]')
        .prop('readonly', !isChecked);
});

$(document).on('change','.form_feilds',function(){
    $("#search-btn").removeClass('d-none')
});

$(document).on('keyup', '.charges_inp', function () {
    count_total();
});
...
const count_total = () => {
    ...
    $('.invoiceTabel tbody tr').each(function () {
        ...
        totalRtoCharges += parseFloat($(this).find('td').find('input[name="rto_charges[]"]').val()) || 0;
        totalHandling += parseFloat($(this).find('td.handling').text()) || 0;
        totalGST += parseFloat($(this).find('td.gst').text()) || 0;
        totalSST += parseFloat($(this).find('td.sst').text()) || 0;
        totalBAC += parseFloat($(this).find('td.bac').text()) || 0;
    });
    $('.total-row').text(totalRows)
    $('.total-cod').text(totalCod)
    $('.total-service').text(totalServiceCharges)
    $('.total-rto').text(totalRtoCharges)
    $('.total-cash').text(totalHandling)
    $('.total-gst').text(totalGST)
    $('.total-sst').text(totalSST)
    $('.total-bac').text(totalBAC)
}