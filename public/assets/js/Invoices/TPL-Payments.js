...
$(document).ready(function () {
    ...
    $(document).on('click', '#generate_tpl_payment', function () {
        var checkedValues = [];

        $('.check_box:checked').each(function () {
            checkedValues.push($(this).val());
        });
        var text = "Are you sure you want to generate tpl payment ?";
        swal.fire({
            title: 'Confirmation',
            text: text,
            type: "question",
            ...
            t.value && $.ajax({
                url: BASEURL + 'add-thirdparty-payments',
                type: 'POST',
                data: {
    ...
    $(document).on('click', '#import_btn', function (e) {
        ...
        if (forms.parsley().isValid()) {
            ...
            $.ajax({
                url: BASEURL + 'import-tpl-payments',
                type: 'POST',
                data: formdata,
                processData: false,
                contentType: false,
...
const btn_html = `
<div id="append_btn_div" class="">
    <div class="btn-group btn-group-split py-2" id="append_btn_div">
        <div id="create-btn-div" class="icn-container float-left pl-4">
            <div class="hover-icon" id="hover-icon-1-584">
                <button type="button" id="generate_tpl_payment"
                    class="btn btn-sm btn-secondary-sm mr-2 " style="background-color: #e4e6eb"
                    data-toggle="tooltip" data-original-title="Generate Payment"><i
