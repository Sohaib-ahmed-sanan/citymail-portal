...
$(document).on('click', '#save-btn', function (e) {
    ...
    e.preventDefault();
    if (forms.parsley().isValid()) {
        var formdata = new FormData($("#insert_de_manifist")[0]);
        var seal_no = $('#seal_no').val();
...
$(document).on('click', '.rem_row', function () {
    ...
    if ($('tbody tr').length === 0) {
        ...
        $("#seal_no").removeAttr('readonly', true);
    }
});
...
$('#update-btn').click(function (e) {
    var forms = $("#search_form");
    var title = $(this).html()
    initLoader('update-btn', title, 'btn-orio');
    forms.parsley().validate();
    if (!forms.parsley().isValid()) {
        destroyLoader('update-btn', title, 'btn-orio')
        return false;
    }
    e.preventDefault();
    if (forms.parsley().isValid()) {
        $("#update-btn").attr("disabled", true).text("Please Wait....");
...
let get_shipments = (seal_no='',shipment_no='') => {
    ...
    $.ajax({
        ...
        url: BASEURL + 'fetch-demanifist-data',
        type: "POST",
        data: {
            seal_no: seal_no,
        ...
        success: function (result) {
            if (result.status == 1) {
                $("#def_msg").remove();
                $("#seal_no").attr('readonly', true);
                $('#shipment_no').val('');
                $("#batch_name").parent().removeClass('d-none');
                $("#batch_name").val(result.batch_name);
                $("#demanifist_list").append(result.payload);
                $("#insert_de_manifist").append(result.form);
