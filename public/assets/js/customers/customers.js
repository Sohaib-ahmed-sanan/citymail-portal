...
$("#addCustomer").click(function (e) {
    ...
    e.preventDefault();
    if (forms.parsley().isValid()) {
        initLoader('addCustomer', 'btn btn-orio');
        var formdata = new FormData($("#customer_form")[0]);
        if ($('#tariff_hidden_feilds input').length > 0 || id != '') {
            $.ajax({
                url: storeUrl,
                type: "POST",
                data: formdata,
                ...
                success: function (result) {
                    if (result.status == 1) {
                        ...
                        var input_case = $('#case').val();
                        if (input_case == 'profileUpdate') {
            ...
            notify("danger", "Please define tarif");
...
$('#assign_courier').change(function () {
    var value = $(this).val();
    if (value == '1') {
        $('#service_tarif').addClass('d-none');
        $('#courier_tarif').removeClass('d-none');
        // $.ajax({
        ...
        $('#service_tarif').removeClass('d-none');
        $('#courier_tarif').addClass('d-none');
    }
});



function filterServiceNames(selectedServiceIds) {
    
    // Iterate over each service_name select
    $('.service_name').each(function() {
        var serviceSelect = $(this);
        
        // Show/hide options based on selectedServiceIds
        serviceSelect.find('option').each(function() {
            var option = $(this);
            console.log(selectedServiceIds.includes(option.val()));
            if (selectedServiceIds.includes(option.val())) {
                option.show();
            } else {
                option.hide();
            }
...
// $(document).on('change', '#services_ids', function() {
...
//         var selectedCode = $(this).data('service-code');
...
//             serviceSelect.append('<option value="' + option.id + '" data-service-code="' + option.code + '">' + option.text + '</option>');
//         });
//     });
...
$(document).on('change', '#services_ids', function() {
    ...
    $('.service_name').each(function() {
        var serviceSelect = $(this);
        var currentSelectedValue = serviceSelect.val();
        serviceSelect.empty();
        serviceSelect.append('<option value="">Select Service</option>'); // Add default option
        options.forEach(function(option) {
            serviceSelect.append('<option value="' + option.id + '" data-service-code="' + option.code + '">' + option.text + '</option>');
        });
        if (currentSelectedValue && serviceSelect.find('option[value="' + currentSelectedValue + '"]').length) {
            serviceSelect.val(currentSelectedValue);
        }
    });
});
