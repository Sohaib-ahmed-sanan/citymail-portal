let datalist = (params) => {
    ...
    var third_party = $("#third_party option:selected").val();
    var courier_id = $("#courier_id option:selected").val();
...
$(document).ready(function () {
    ...
    var param = {
        ...
        label: '',
        third_party: $("#third_party option:selected").val(),
        courier_id: $("#courier_id option:selected").val(),
    ...
    $('#apply_filters').click(function () {
        $('#append_btn_div').removeClass('d-block').addClass('d-none');
        param.third_party = $("#third_party option:selected").val();
        param.courier_id = $("#courier_id option:selected").val();
        param.customer_id = $("#customer_id option:selected").val();
        if ((param.third_party === '') && (param.courier_id.length === 0) && (param.customer_id.length === 0)) {
            notify("warning", "Oh snap!", "Please select any one option to filter.");
            return false
        } else {
            if (param.third_party == '' || param.third_party == '0' && param.courier_id.length > 0) {
                notify("danger", "Error", "Cannot apply courier filter when third party is not created.");
                return false
            } else {
                var startdate = $("#apply_filters").attr('data-startdate');
                var enddate = $("#apply_filters").attr('data-enddate');
                param.start = moment(startdate);
                param.end = moment(enddate);
                param.value_type = "filter";
                let text = $("#apply_filters").html();
                param.spinerType = `apply_filters|${text}`
                initLoader('apply_filters', text, 'btn-orio');
                datalist(param);
    ...
    $("#resetFilter").click(function () {
        let text = $(this).text();
        initLoader('resetFilter', text, 'btn-secondary-orio');
        $('#third_party').val('').trigger('change');
        ...
        param.customer_id = '';
        param.value_type = "filter";
        param.spinerType = `resetFilter|${text}`
        param.start = moment().subtract(3, 'days');
        param.end = moment();
        init_datepicker(picker, param.start, param.end, datalist);
        datalist(param);
    });
...
// to get and display the data of selected cn in modal
$(document).on('click', '.open_modal', function () {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: {
            cn_numbers: checkedValues
        },
        success: function (result) {
            if (result.status == 1) {
                $('#display-data').html(result.html);
            } else {
...
$(document).on('click', '.cn_void', function () {

    let title = $(this).text();
    initLoader('cn_void_btn', title, 'btn-secondary-sm');
    var checkedValues = [];
    $('.check_box:checked').each(function () {
        var tpl_consignment = $(this).val();
        var courier_id = $(this).data('courier-id');
        var account_id = $(this).data('courier-account-id');
        checkedValues.push({
            'account_id': account_id,
            'courier_id': courier_id,
            'tpl_consignment': tpl_consignment
        });
    });

    swal.fire({
        title: 'Confirmation',
        text: 'Are you sure you want to void selected consigments?',
        type: 'question',
        ...
        t.value && $.ajax({
            ...
            success: function (result) {
                if (result.status == 1) {
                    ...
                    destroyLoader('cn_void_btn', '<i class="fas fa-trash-alt"></i>', 'btn btn-secondary-sm');
                } else {
                    notify("danger", "Oh snap!", result.message);
                    destroyLoader('cn_void_btn', '<i class="fas fa-trash-alt"></i>', 'btn btn-secondary-sm');
                }
            ...
            error: function (xhr, err) {
                notify("warning", "Oh snap!", "Something went wrong");
                destroyLoader('cn_void_btn', title, 'btn btn-secondary-sm');
            }
        });
    });
...
//         $.ajax({
//             url: BASEURL + 'tpl-cn-print-bulk',
//             type: 'POST',
//             data: {
//                 bulk_array: checkedValues
//             },
//             success: function (result) {
//                 if (result.status == 1) {
//                     var base64Pdf = result.pdf_content;
...
//         });
//     } else {
//         notify("danger", "Sorry", "This courier doesnt support bulk print");
//     }
// });

// $(document).on('click', '.created_bulk_print', function () {
//     var checkedValues = [];
//     $('.created_check_box:checked').each(function () {
//         var tpl_consignment = $(this).val();
//         var courier_id = $(this).data('courier-id');
//         var account_id = $(this).data('courier-account-id');
//         if (courier_id == '22') {
...
$('#courier_acc_id').change(function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            $('#courier_code').empty();
            if (result.length > 0) {
                $('#selectCourier-row').removeClass('d-none');
                result.forEach(function (item) {
                    $('#courier_code').append('<option value="' + item.code + '">' + item.code + '</option>');
                });
            }
        },
        error: function (xhr, err) {
...
$("#add-btn").click(function (e) {
    let title = $(this).text();
    initLoader('add-btn', title, 'btn-secondary-orio');
    var forms = $("#add_tpl_form");
    var store_url = $("#add_url").val();
    var courier_id = $("#courier_acc_id option:selected").data('courier-id');
    let cn_numbers = [];
    $('#display-data tr').each(function () {
        const number = $(this).find('td').eq(0).text().trim();
        if (!isNaN(number) && number !== '') {
            cn_numbers.push(Number(number));
        }
    });
    ...
    e.preventDefault();
    if (forms.parsley().isValid()) {
        var formdata = new FormData($("#add_tpl_form")[0]);
        formdata.append('courier_id', courier_id);
        formdata.append('cn_numbers', cn_numbers);
        $.ajax({
            url: store_url,
            type: "POST",
            data: formdata,
            ...
            timeout: 800000,
            success: function (result) {
                ['#insurance', '#type_id', '#fragile', '#courier_code', '#courier_acc_id'].forEach(id => {
                    $(id).val('').trigger('change');
                });
                if (result.status == 1) {
                    destroyLoader('add-btn', title, 'btn btn-orio')
                    $("#openPopup").modal("hide");
                    $("#add_tpl_form")[0].reset();
                    $('#result-modal').modal('show')
                    var error_html = success_html = '';
                    $('#display-success, #display-error').html('');
                    $('#error_count').addClass('d-none')
                    if (result.error_array.length > 0) {
                        var i = 1;
                        result.error_array.forEach((error) => {
                            error_html += `
                                <tr>
                                    <td>${i++}</td>
                        ...
                        $('#display-error').html(error_html);
                        var count = $('#display-error').find('tr').length
                        $('#error_count').removeClass('d-none').text(count);
                    }
                    if (result.success_array.length > 0) {
                        var i = 1;
                        result.success_array.forEach((success) => {
                            let encodedReference = btoa(success.reference);
                            let airway_url = `${BASEURL}airway-pdf/${encodedReference}`;
                           success_html += `
                                <tr>
                                    <td>
                                        <div class="custom-control custom-checkbox my-3">
                                            <input type="checkbox" class="custom-control-input created_check_box" data-type="others" 
                                                   name="checked_id[]" data-courier-account-id="${success.customer_courier_id}" 
                                                   data-courier-id="${success.courier_id}" data-consignment="${success.reference}" 
                                                   value="${success.tpl_consignment_no}" id="print_check_${i}">
                                            <label class="custom-control-label" data-toggle="tooltip" title="Check to print" 
                                                   for="print_check_${i}"></label>
                                        </div>
                                    </td>
                                    <td>${i++}</td>
                                    <td>${success.tpl_consignment_no}</td>
                                    <td width="25%">${success.reference}</td>
                                    <td>${success.message}</td>
                                    <td>
                                        ${success.courier_id !== '24' 
                                            ? `<a style="color: #ba0c2f !important;" href="${BASEURL + 'tpl-cn-print/' + success.courier_id + '/' + success.customer_courier_id + '/' + success.tpl_consignment_no}" 
                                                 target="_blank" data-courier-id="${success.courier_id}" 
                                                 data-courier-account-id="${success.customer_courier_id}" 
                                                 data-cn="${success.tpl_consignment_no}" 
                                                 data-route="${BASEURL + 'tpl-cn-print/' + success.courier_id + '/' + success.customer_courier_id + '/' + success.tpl_consignment_no}" 
                                                 data-toggle="tooltip" title="TPL Print">
                                                 <img src="${BASEURL + 'images/default/svg/print.svg'}" width="15" alt="TPL Print">
                                               </a>` 
                                            : ''}
                                        <a style="color: #ba0c2f !important;" href="${airway_url}" 
                                           target="_blank" data-toggle="tooltip" title="System Print">
                                            <img src="${BASEURL + 'images/default/svg/print.svg'}" width="15" alt="System Print">
                                        </a>
                                    </td>
                                </tr>
                            `;
                        ...
                        $('#display-success').html(success_html);
                        $("#example").DataTable().ajax.reload();
                        notify("success", "TPL Shipments has been created");
                    }
                } else {
                    var i = 1;
                    var html = '';
...
$(document).on('click', '.single-track', function () {
    var consignment_no = $(this).data('cn');
    var tpl_cn_number = $(this).data('tpl-cn');
    var courier_id = $(this).data('courier-id');
    var account_id = $(this).data('courier-account-id');
    var track_route = $(this).data('route');
    $.ajax({
        url: track_route,
        type: 'POST',
        data: {
            consignment_no: consignment_no,
            tpl_cn_number: tpl_cn_number,
            courier_id: courier_id,
            account_id: account_id,
        },
        success: function (result) {
            if (result.status = 1) {
                ...
                $('.track-details').html(result.status_list);
                $('.cn_div').html(tpl_cn_number);
                $('.main_detail').html(result.modal);
            }
        },
        error: function (xhr, err) {
            notify("warning", "Oh snap!", "Something went wrong");
...
// for the checkboxes work checkboxes
$(document).on('click', '.check_box', function () {
    check_all();
});

const btn_html = `
    <div class="btn-group btn-group-split py-2 d-none" id="append_btn_div">
        <div id="create-btn-div" class="icn-container float-left pl-4 d-none">
            <div class="hover-icon" id="hover-icon-1-584">
                <a href="javascript:void(0);" class="open_modal btn btn-sm btn-secondary-sm mr-2 "
                    style="background-color: #e4e6eb" data-toggle="tooltip"
                    data-original-title="Create Third Party"><i class="fas fa-truck"></i></a>
            </div>
        </div>
        <div id="void-btn-div" class="icn-container float-left pl-4 d-none">
            <div class="d-flex">
                <div class="hover-icon" id="hover-icon-1-584">
                    <button type="button" class="btn btn-sm btn-secondary-sm mr-2 cn_void"
                        style="background-color: #e4e6eb" id="cn_void_btn" data-toggle="tooltip"
                        data-original-title="Consignment void"><i class="fas fa-trash-alt"></i></button>
                </div>
                <div class="hover-icon" id="hover-icon-1-584">
                    <button type="button" onclick="bulk_print('.check_box')" class="bulk_print btn btn-sm btn-secondary-sm mr-2 "
                        style="background-color: #e4e6eb" data-toggle="tooltip"
                        data-original-title="TPL Bulk Print"><i class="fas fa-print"></i></button>
                </div>
            </div>
        </div>
    </div>
...
$("#check_all_created").click(function () {
    if ($(this).prop('checked') == true) {
        $(".created_check_box").prop('checked', true);
    } else {
        $(".created_check_box").prop('checked', false);
    }
});

let check_all = (elem) => {
    var dataType = $('.check_box:checked').first().data('type');
    others = false;
    create = false;
    if (dataType === 'others') {
        others = true;
    }
    if (dataType === 'creation') {
        create = true;
    }
    ...
    if (l > 0) {
        ...
        $('#append_btn_div').removeClass('d-none').addClass('d-block').slideDown();
        if (others) {
            $('#void-btn-div').removeClass('d-none').slideDown();
            $('#create-btn-div').addClass('d-none').slideDown();
        }
        if (create) {
            $('#create-btn-div').removeClass('d-none').slideDown();
            $('#void-btn-div').addClass('d-none').slideDown();
        }
    } else {
        $('#append_btn_div').slideUp().removeClass('d-block').addClass('d-none');
    }
};
...
$(document).on('click', '.cn_create', function () {
