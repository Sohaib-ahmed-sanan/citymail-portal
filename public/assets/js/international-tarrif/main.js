let datalist = (params) => {
    ...
    $("#example").DataTable({
        ...
        oLanguage: {
            sSearch: "",
            sSearchPlaceholder: "Search Tarrif",
            oPaginate: {
...
const btn_html = `
    <div class="btn-group btn-group-split" id="append_btn_div">
        <div id="create-btn-div" class="icn-container px-2 py-2 float-left pl-4">
            <div class="hover-icon" id="hover-icon-1-584">
                <a href="javascript:void(0);" id="update_tariff"
                    class="btn btn-sm btn-secondary-sm mr-2" style="background-color: #e4e6eb"
                    data-toggle="tooltip" data-original-title="Update tariff">
                    <i class="fa-solid fa-dolly"></i></a>
            </div>
        </div>
    </div>
`;
...
let check_all = () => {
    ...
    mylabel.each(function () {
        var checkedCount = $('.dataTables_wrapper table').find('.check_box:checked').length;
        if (checkedCount > 0) {
