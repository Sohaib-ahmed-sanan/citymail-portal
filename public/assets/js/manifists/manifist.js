        ...
        if (!consignmentExists) {
            ...
            if (rowCount <= 99) {
                $.ajax({
                    ...
                    url: BASEURL + 'fetch-manifist-data',
                    type: "POST",
                    data: {
...
$(document).on('click', '#save-btn', function (e) {
    ...
    if (forms.parsley().isValid()) {
        ...
        var cn_numbers = [];
        if (batch_name != '' && station_id != '' && rider_id != '' && seal_no != '') {
            $('#arrival_list tr').each(function () {
                var consignment = $(this).find("td:eq(0)").text().trim();
            ...
            notify("danger", "Error", "Please provide complete information");
        }
    }
});
...
$(document).on('click', '.rem_row', function () {
    ...
    if ($('#arrival_list tbody tr').length === 0) {
