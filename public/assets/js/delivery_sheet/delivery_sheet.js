...
$('#shipment_no').on('input', function (e) {
    var val = $(this).val();
    if (val.length == 9) {
        var shipment_no = $('#shipment_no').val();
        let detail = [];
        $('#delivery_list tr').each(function () {
            var consignment = $(this).find("td:eq(0)").text().trim();
            detail.push(consignment);
        });
        if (!detail.includes(shipment_no)) {
            var forms = $("#search_form");
            forms.parsley().validate();
            if (!forms.parsley().isValid()) {
                $("#search-btn").removeAttr("disabled", true).text("Search")
                return false;
            }
            e.preventDefault();
            if (forms.parsley().isValid()) {
                ...
                $.ajax({
                    ...
                    url: BASEURL + 'fetch-delivery-data',
                    type: "POST",
                    data: formdata,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (result) {
                        if (result.status == 1) {
                            ...
                            $("#delivery_list").append(result.payload);
                            $("#insert_delivery_sheet").append(result.form);
                            initSelect2();
                        } else {
...
$(document).on('click', '#save-btn', function (e) {
    ...
    let currencyGroups = {};
    var title = $(this).text();
    allStatusesFilled = true;
    initLoader('save-btn', title, 'btn-secondary-orio');

    $('#delivery_list tr').each(function () {
        var consignment = $(this).find("td.cn_no").text().trim();
        var currency_code = $(this).find("td.currency_code").text().trim();
        var status = $(this).find("select[name='status_single[]']").val();
        if (status != "") {
            var remarks = $(this).find("input[name='remarks[]']").val();
            if (!currencyGroups[currency_code]) {
                currencyGroups[currency_code] = [];
            }
            currencyGroups[currency_code].push({
                "consignment": consignment,
                "status": status,
                "remarks": remarks
            });
        }else{
            allStatusesFilled = false;
            notify("danger","Error","Please select status for all shipments");
    ...
    if (riders_id != '' && route_id != '' && allStatusesFilled == true) {
        if (Object.keys(currencyGroups).length === 1) {
            swal.fire({
                title: 'Confirmation',
                text: 'Are you sure you want to generate delivery sheet',
                type: "question",
                buttonsStyling: !1,
                ...
                t.value && generate_sheet(currencyGroups, rider_id, route_id);
            ...
            swal.fire({
                ...
                text: 'There are multiple currencies. Do you want to generate separate delivery sheets for each currency?',
                type: "warning",
                buttonsStyling: !1,
                showCancelButton: true,
                confirmButtonText: 'Yes, create seperate sheet!',
                cancelButtonText: 'No, cancel',
                confirmButtonClass: "btn btn-orio",
                cancelButtonClass: "btn btn-secondary-orio mx-3"
            }).then(t => {
                t.value &&
                    generate_sheet(currencyGroups, rider_id, route_id);
                    destroyLoader('save-btn', title, 'btn btn-secondary-orio')
            });
        }
    }else{
...
const generate_sheet = (detail, rider_id, route_id) => {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.status == 1) {
                swal.fire({
                    icon: "success",
                    title: "DeliverySheet sheet no # " + result.payload,
                    text: "DeliverySheet has been created",
                    confirmButtonClass: "btn-success",
                    type: "success",
                });
                window.setTimeout(function () {
                    window.location.href = BASEURL + 'delivery-sheet';
                }, 2000);
            } else {
...
$('#update-btn').click(function (e) {
    initLoader('update-btn', 'btn-orio')
    let detail = [];
    $('#delivery_list tr').each(function () {
        var consignment = $(this).find("td:eq(0)").text().trim();
        var status = $(this).find("select[name='status_id[]']").val();
        var remarks = $(this).find("input[name='remarks[]']").val();
        var current_status = $(this).find("input[name='current_status[]']").val()
        if (!['14', '17'].includes(current_status) && status != null) {
            detail.push(
                {
                    "consignmet": consignment,
                    "status": status,
                    "remarks": remarks
                }
            );
    ...
    if (detail.length > 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status == 1) {
                    ...
                    destroyLoader('update-btn', "Update", 'btn btn-orio')
                }
            },
            error: function (xhr, err) {
                var errorMessage = xhr.responseJSON.message;
        ...
        notify("danger", 'Error', 'Please provide valid data');
        destroyLoader('update-btn', "Update", 'btn btn-orio')
    }
});
...
$('#status_all').change(function () {
    ...
    $('.status_single').val(status_id).trigger('change');
});