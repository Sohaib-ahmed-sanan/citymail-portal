...
let tabs_tabels = (params) => {
    ...
    var activeTabelId = $('.tabs-links[aria-selected="true"]').attr('id');

    if (activeTabelId == 'return-request-tab') {
        var aoColumns = [{
                data: "CHECK"
            },
            {
        ...
        var shipmentStatus = '15';
        var tableId = "#return_request";
    } else if (activeTabelId == 'returns-tab') {
        var shipmentStatus = '16';
        var tableId = "#return";
        var aoColumns = [{
                data: "CHECK"
            },
...
const initializeDataTable = ({
    ...
    $(tableId).DataTable({
        ...
        responsive: true,
        bAutoWidth: true,
        // dom: '<"row py-3"<"col-md-7 applyall"><"col-md-5 d-flex align-items-center justify-content-end" fB>><t><"table-footer-wrapper" <"divider"> <"row"<"col-md-6 d-flex align-items-center" il><"col-md-6 d-flex align-items-center justify-content-end" p>>>',
        dom: '<"row py-3"<"col-md-7 applyall"><"col-md-5 d-flex align-items-center justify-content-end" fB>><t><"table-footer-wrapper" <"row"<"col-md-6 d-flex align-items-center" il><"col-md-6 d-flex align-items-center justify-content-end" p>>>',
        buttons: [{
                extend: "excelHtml5",
...
$(document).ready(function () {
    ...
    var param = {
        ...
        label: '',
        customer_acno: $("#customer_filter").val(),
        value_type: "",
        spinerType: "",
    }

    init_datepicker(picker, param.start, param.end, tabs_tabels);
    tabs_tabels(param);
    
    $(document).on('click', '.tabs-links', function () {
        if ($(this).attr('aria-selected') == 'true') {            
            var startDate = moment($('#apply_filters').attr('data-startdate'));
            var endDate = moment($('#apply_filters').attr('data-enddate'));
            var param = {
                start: startDate,
                end: endDate,
                label: '',
    ...
    $('#apply_filters').click(function () {
        param.customer_acno = $("#customer_filter").val();
...
$(document).on('click', '.chat_modal', function () {
    var consignment_no = $(this).data('consignment_no');
    load_chats(consignment_no)
});

const sendMessage = () => {
    var msg = $('#getMessage').val();
    var consignment_no = $('#get_consigment_no').val();
    if (msg.length > 0) {
        $.ajax({
            url: BASEURL + 'store-shipper-advice',
            type: 'POST',
            data: {
        ...
        notify("error", "Error", "Cannot send empty message");
    }
}

const load_chats = (consignment_no) => {
    $.ajax({
        url: BASEURL + 'chats-shipper-advice',
        type: 'POST',
        data: {
            consignment_no: consignment_no
        },
        success: function (result) {
            if (result.status == 1) {
                $('#appendMessages').html('');
                $('#get_consigment_no').val(consignment_no);
                $('.cn_no').text(consignment_no);
                $('#appendMessages').html(result.payload);
