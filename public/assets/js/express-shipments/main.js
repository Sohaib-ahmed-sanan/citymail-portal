let datalist = (params) => {
    ...
    $("#example").DataTable({
        ...
        oLanguage: {
            sSearch: "",
            sSearchPlaceholder: "Search Shipments",
            oPaginate: {
...
$(document).ready(function () {
    ...
    $('#apply_filters').click(function () {
        param.destination_id = $("#destination_id").val();
        param.status_id = $("#status_id").val();
        param.country_id = $("#country_id").val();
        if ((param.country_id == "") && (param.destination_id == '') && (param.status_id == '')) {
    ...
    $("#resetFilter").click(function () {
        ...
        param.destination_id = param.country_id = '';
        param.value_type = "filter";
...
$("#save-btn").click(function (e) {
    var forms = $("#shipment_form");
    var storeUrl = forms.data("url");
    ...
    if (forms.parsley().isValid()) {
        ...
        let details = [];
        $("#detais_table tr").each(function () {
            let row = $(this);
            let id = row.attr('data-id');            
            let product_detail = getRowValue(row, ".product_detail", "product_detail_"+id);
            let hs_code = getRowValue(row, ".hs_code", "hs_code_"+id);
            let value = getRowValue(row, ".value", "value_"+id);
            let qty = getRowValue(row, ".qty", "qty_"+id);
            let amount = getRowValue(row, ".amount", "amount_"+id);
            if (product_detail === "" || hs_code === "" || value === "" || qty === "" || amount === "") {
                notify("danger","Please provide at least 1 product detail");
                destroyLoader('save-btn', title, 'btn-orio')
                return false; 
            }
            details.push({
            ...
            success: function (result) {
                if (result.status == 1) {
                    swal.fire({
                        icon: "success",
                        title: 'Success',
                        text: result.message +' '+ result.payload.consignment_no,
                        confirmButtonClass: "btn-success",
                        type: "success",
                    });
                    window.setTimeout(function () {
                        window.location.href = BASEURL + 'express-shipments';
                    }, 2000);
                } else {
...
let addDetails = (e) => {    
    ...
    $('.default').remove()
    let count = $('#detais_table tr').length +1;
    let productDetails = $('#product_details_inp').val();
    let hsCode = $('#hs_code_inp').val();
    let value = parseFloat($('#value_inp').val()) || 0;
    let qty = parseInt($('#qty_inp').val()) || 0;
    ...
    let rowsHtml = details.map(val => `
        <tr>
            <td class="">${count}</td>
            <td class="product_detail">${val.product_detail}</td>
            <td class="hs_code">${val.hs_code}</td>
            <td class="value">${val.value}</td>
            <td class="qty">${val.qty}</td>
            <td class="d-none amount">${val.amount}</td>
            <td>
                <button type="button" class="btn btn-danger p-2 btn-sm remove_row" style="min-width: 10px !important" data-count="${count}"><i class="fa-solid fa-trash"></i></button>
            </td>
        </tr>
        `).join('');
    $('#detais_table').append(rowsHtml);
};
...
let getRowValue = (row, selector, inputName) => {
    const input = row.find(`input[name='${inputName}']`);
    if (input.length > 0) {
...
let create_pickup = (element,e) => {
    var forms = $("#add_location_from");
    var storeUrl = forms.data("url");
    forms.parsley().validate();
    var title = $(element).html()
    initLoader(element.id, title, 'btn-orio');
    if (!forms.parsley().isValid()) {
        destroyLoader(element.id, title, 'btn-orio')
        return false;
    }
    e.preventDefault();
    if (forms.parsley().isValid()) {
        var originalFormData = new FormData($("#add_location_from")[0]);
        var formdata = new FormData();

        // Renaming fields
        formdata.append("name", originalFormData.get("pickup_name"));
        formdata.append("title", originalFormData.get("pickup_title"));
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status == 1) {
                    $("#add_location_from")[0].reset();
                    $("#add_pickup_modal").modal("hide");
                    swal.fire({
                        icon: "success",
                        title: 'Success',
                        text: result.message,
...
let create_customer = (element,e) => {
    var forms = $("#add_customer_from");
    var storeUrl = forms.data("url");
    forms.parsley().validate();
    var title = $(element).html()
    initLoader(element.id, title, 'btn-orio');
    ...
    e.preventDefault();
    if (forms.parsley().isValid()) {
        var formdata = new FormData($("#add_customer_from")[0]);
        $.ajax({
            url: storeUrl,
            type: "POST",
            data: formdata,
...
let get_walkin_customer = () => {
    $.ajax({
        url: BASEURL + 'get-walkin-customer',
        type: "POST",
        data: {
         type:"void"   
        },
        success: function (result) {
            if(result.status == 1){
                ...
                result.payload.forEach(val => {
                    html += ` <option value="${val.acno}">
                                    ${val.business_name}
                                    (${val.acno})
                                </option>` 
                });
                $('#customer_acno').html(html)
            }
        }
    });
}
