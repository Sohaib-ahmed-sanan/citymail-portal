let datalist = (params) => {
    ...
    // Check if sessionType is in the specified array
...
$(document).ready(function () {
    ...
    $('#apply_filters').click(function () {
        param.customer_id = $("#customer_filter").val();
        param.destination_id = $("#destination_id").val();
        param.status_id = $("#status_id").val();
        param.country_id = $("#country_id").val();
        // datalist(start, end, "",customer_id,destination_id);
        if ((param.customer_id == '') && (param.country_id == "") && (param.destination_id == '') && (param.status_id == '')) {
    ...
    $("#resetFilter").click(function () {
        ...
        param.customer_id = param.destination_id = param.country_id = '';
        param.value_type = "filter";
...
$("#addCustomer").click(function (e) {
    ...
    if (!forms.parsley().isValid()) {
        destroyLoader('addCustomer', title, 'btn btn-orio')
        return false;
    }
    e.preventDefault();
    if (forms.parsley().isValid()) {
...
$(document).on('click', '#generate_loadSheet', function () {
    ...
    if (checkedValues.length > 0) {
        ...
        var text = "Are you sure you want to create load sheet?";
...
$("#cancle-btn").click(function (e) {
    ...
    if (forms.parsley().isValid()) {
        ...
        swal.fire({
            title: 'Confirmation',
            text: 'Are you sure you want to void the shipments',
            type: "question",
            buttonsStyling: !1,
            showCancelButton: true,
            confirmButtonText: 'Yes, create it!',
