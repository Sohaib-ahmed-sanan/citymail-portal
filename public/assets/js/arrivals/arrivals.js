            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.status == 1) {
                        ...
                        // $(".wp-col").removeClass('d-none');
                        // $("#weight-first").removeClass('d-none').val(result.weight);
                        // $("#peices-first").removeClass('d-none').val(result.peices);
...
$(document).on('click', '#save-btn', function () {
    ...
    if (rider_id != '' && station_id != '' && route_id != '') {
        ...
        swal.fire({
            title: 'Confirmation',
            text: 'Are you sure you want to generate pickup',
            type: "question",
            buttonsStyling: !1,
            ...
            t.value && $.ajax({
                ...
                success: function (result) {
                    if (result.status == 1) {
                        swal.fire({
                            icon: "success",
                            title: "Pickup sheet no # " + result.sheet_no,
                            text: result.message,
                        ...
                        destroyLoader('save-btn', title, 'btn btn-orio')
