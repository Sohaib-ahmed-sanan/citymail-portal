...
$(document).ready(function () {
    ...
    $('#apply_filters').click(function () {
        ...
        param.sub_account_id = $("#sub_account_id").val();
        if ((param.customer_acno == '') && (param.city_id == '')) {
            ...
            initLoader('apply_filters', text, 'btn btn-orio');
            datalist(param);
    ...
    $("#resetFilter").click(function () {
        let text = $(this).text();
        initLoader('resetFilter', text, 'btn btn-secondary-orio');
        $('#customer_filter').val('').trigger('change');
        $("#sub_account_id").val('').trigger('change');
        $('#city_id').val('').trigger('change');
        param.customer_acno = param.city_id = '';
        param.value_type = "filter";
...
$("#add-btn").click(function (e) {
    ...
    initLoader('add-btn', 'btn btn-orio');
...
$(document).on("change", "#courier", function () {
    ...
    var country_code = $("#country_id option:selected").attr("data-attr");
    let data = {
        courier: courier,
        country: country,
        country_code: country_code,
    };
    $.post(BASEURL + "get-courier-city" , data ,function(row){
        var html = "";
        if(row.payload.length !=0){
            for (let i = 0; i < row.payload.length; i++) {
                const e = row.payload[i];
                html += `<option value="${e.city_code}"> ${e.city_name} </option>`;
            }
        }
        $("#city_courier").html(html);
    });

    // $.ajax({
    //     url: BASEURL + "get-courier-city", //this is for common controller
    //     method: "POST",
    //     data: {
    //         courier: courier,
    //         country: country,
    ...
    //         var html = "";
    //         res.payload.each(function (index,city) {
