    ...
    $("#example").DataTable({
        ...
        drawCallback: function (settings) {
            ...
            if (value_type === 'filter' && response) {
                let [action, text] = spinerType.split('|');
                let btn_class = action === 'resetFilter' ? 'btn btn-secondary-orio' : 'btn btn-orio';
                destroyLoader(action, text, btn_class);
            }
            $('#apply_filters').attr({
...
$(document).ready(() => {
    ...
    $('#apply_filters').click(() => {
        ...
        if (
            param.customer_acno.length < 0 &&
            param.sub_account_id.length < 0 &&
            param.country_id.length < 0 &&
            param.destination_id.length < 0 &&
            param.city_id.length < 0 &&
            param.status_id.length < 0
        ) {
            notify("warning", "Oh snap!", "Please select at least one filter option.");
            return;
        }

        param.start = moment($("#apply_filters").attr('data-startdate'));
        param.end = moment($("#apply_filters").attr('data-enddate'));
        param.value_type = "filter";
        param.spinerType = `apply_filters|${$("#apply_filters").html()}`;

        initLoader('apply_filters', param.spinerType.split('|')[1], 'btn-orio');
    ...
    $("#resetFilter").click(() => {
        let text = $("#resetFilter").text();
        initLoader('resetFilter', text, 'btn-secondary-orio');
        $('#customer_filter, #sub_account_id, #city_id').val('').trigger('change');
        param = {
            start: moment().subtract(3, 'days'),
            end: moment(),
            label: '',
