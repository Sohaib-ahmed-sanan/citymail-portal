...
@section('content')
                        ...
                        <div class="col-md-12">
                            <form action="#" data-parsley-validate id="report_form" method="post">
                                <div class="form-row flex-column justify-content-center align-items-center">
                                    <div class="col-md-10 mb-3 px-3">
                                        @if (is_ops())
                                            <select required
                                                ...
                                                id="customer_acno" data-select2-id="1" multiple data-toggle="select2"
                                                name="customer_acno[]" size="1" label="Select Customer"
                                                data-placeholder="Select Customer" data-allow-clear="0">
                                        ...
                                        @else
                                            <input type="hidden" id="customer_acno" name="customer_acno[]"
                                                value="{{ session('acno') }}">
                                        @endif
                                        @if (is_portal())
                                            @if (count($sub_acc) > 0)
                                                <select required class="form-control form-select sub_acc_id form-control-lg"
                                                    id="sub_acc" multiple="multiple" data-select2-id="2"
                                                    data-toggle="select2" name="sub_acc[]" size="2"
                                                    label="Select Sub Account" data-placeholder="Select Sub Account"
                                                    data-allow-clear="1">
                                    ...
                                    <div class="col-md-10 mb-3 px-3">
                                        <select class="form-select city_id form-control-lg" id="destination_id"
                                            multiple="multiple" data-toggle="select2" name="city_id[]" size="1"
                                            label="Select Destination" data-placeholder="Select Destination"
                                            data-allow-clear="1" data-select2-id="4">
                                    ...
                                    <div class="col-md-10 mb-3 px-3">
                                        <select class=" form-control form-select destination_id form-control-lg input_field"
                                            id="status_id" data-select2-id="5" data-toggle="select2" multiple="multiple"
                                            name="status_id[]" size="1" label="Select Delivery Status"
    ...
    <script>
        var listRoute = "{{ route('admin.revenue_details') }}";
        var sessionType = "{{ session('type') }}";
    </script>
@section('scripts')
    <script src="{{ asset('assets/js/reports/revene_details.js') }}"></script>
@endsection
@endsection
